#PURPOSE:	this program will compute the value of 2³ + 5² 
#
...
pushl %eax		#save the first answer before callin the next function
...
pop %ebx 		#the second answer is already in %eax. We saved the first answer onto the
			#stack, so now we can just pop it out into %ebx

addl %eax, %ebx		#add them together. The result es in %ebx
...
#			%eax		is used for termporary storage
#
...
subl $4, %esp		#get foom for our local storage
